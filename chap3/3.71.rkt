...
(define (cube x) (* x x x))
(define (sum-triple pair)
  (let ((i (car pair))
        (j (cadr pair)))
    (+ (cube i) (cube j)))) 

#| Answer |#
(define ex (weighted-pairs integers integers sum-triple))

(define (make-rama stream)
  (cond ((stream-null? stream) the-empty-stream)
        ((= (sum-triple (stream-car stream)) (sum-triple (stream-car (stream-cdr stream))))
         (cons-stream (list (stream-car stream) (stream-car (stream-cdr stream)) (sum-triple (stream-car stream)))
                      (make-rama (stream-cdr stream))))
        (else (make-rama (stream-cdr stream)))))

#|
The first 6 Ramanujan numbers are:
1729
4104
13832
20683
...
(define answer (make-rama ex))
(display-n-terms answer 5)