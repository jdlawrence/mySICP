...
#| Answer |#
(define ex (weighted-pairs integers integers sum-square))
...
(define (answer stream)
  (let ((first (stream-car stream))
    ...
    (cond ((stream-null? stream) the-empty-stream)
          ((= (sum-square first) (sum-square second) (sum-square third))
           (cons-stream (list first second third (sum-square first))
                        (answer (stream-cdr stream))))
          (else (answer (stream-cdr stream))))))

#|
The first numbers meeting this pattern are:
((10 15) (6 17) (1 18) 325)
((13 16) (8 19) (5 20) 425)
...
(define final-answer (answer ex))
(display-n-terms final-answer 5)